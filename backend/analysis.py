...
def detect_date_candidates(
    ...
    for col in sample.columns:
        series = sample[col]
        declared_type = column_types.get(col)
        is_declared_date = declared_type == "date" or is_datetime64_any_dtype(series)
        parsed = pd.to_datetime(series, errors="coerce", dayfirst=True, infer_datetime_format=True)
        parse_ratio = parsed.notna().mean()
        parsed_dates = parsed.dropna()
        unique_calendar_days = parsed_dates.dt.date.nunique() if not parsed_dates.empty else 0

        if declared_type == "numeric" and unique_calendar_days <= 3:
            # Evita interpretar montos como fechas (e.g., 1970-01-01 con horas distintas).
            continue

        if not is_declared_date and parse_ratio < 0.6:
            continue
        if unique_calendar_days == 0:
            continue

        null_ratio = 1 - parse_ratio if len(series) else 1
        distinct_values = int(parsed_dates.nunique())
