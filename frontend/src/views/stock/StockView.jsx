...
const StockView = ({ onUnauthorized }) => {
  ...
  const fetchAutoMetrics = async () => {
    ...
    try {
      ...
      setError(err.message || "No se pudo obtener métricas de stock");
    } finally {
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <SectionHeader title="Stock" subtitle="Rotación, días y riesgo" badge="Panel de inventario" />
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {loading.auto ? (
              ...
              <>
                ...
                <MetricCard label="Stock total" value={kpis.total_stock_units?.value} />
                <MetricCard label="SKU en riesgo" value={kpis.products_with_low_stock?.value} />
              </>
            )}
          </div>

          <div className="grid gap-4 lg:grid-cols-2">
            <ChartCard title="Ranking de rotación" chart={chartData} />
            <ChartCard title="Stock vs ventas" chart={chartData} />
          </div>

          <TableCard title="Tabla de stock" data={tableData} columnTypes={{}} />
        </div>
      ) : (
        <div className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <p className="text-xs text-slate-500">Listo para graficar y mostrar en tabla</p>
          </div>
